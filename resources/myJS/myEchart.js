...
$(function(){
	...
	$(".country > div").click(function(){
		...
		if($($("#chartDiv_1 > .mode")[0]).attr("class").indexOf("tip") >= 0){
			chartYear($.trim($(this).html()),false);
		}else{
	...
	if(country['country'] == undefined){
		...
		chartYear(country['country'],false);
...
var chart ;
//按年
function chartYear(countryName,isPie){
...
//		        shadowColor: 'rgba(0, 0, 0, 0.5)',
	...
	if(total == 0){
		...
		$(".pieProp").css("display","block");
	}else{
		option.color = ['#C1232B'];
		$(".pieProp").css("display","none");
	}
    ...
    var PageEvent = function (i) {
    	var legendLength = chart.getOption().legend[0].data.length;
    	console.log(legendLength);
      	 if(clickCount * 10 > legendLength){
      		clickCount = clickCount - 1;
      		$('.js-nextPage img').attr({'src': 'Images/pieImage/down-disable.png', 'title': '已经是最后一页'});
            $('.js-nextPage img').css('cursor','no-drop');
      		 return;
      	 }else{
      		$('.js-nextPage img').attr({'src': 'Images/pieImage/down-icon.png', 'title': '下一页'});
      	 }
    ...
    if (option.legend.pagemode) {
    	var legendLength = chart.getOption().legend[0].data.length;
    	if(legendLength <= 10 ){
    		$('.js-prePage img').attr({'src': 'Images/pieImage/up-disable.png', 'title': '已经是第一页'});
        ...
        $('body').on('click', '.js-prePage', function () {
            if (clickCount > 0) {
                ...
                $('.js-prePage img').attr({'src': 'Images/pieImage/up-icon.png', 'title': '上一页'});
    ...
    chart.on('legendselectchanged', function(params) {
      ...
      for(var i=0;i<select_value.length;i++){
 	   if(select_value[i] == false){
 		 index = i;
 		clickName = option.series[0].data[i].name;
 		clickNum = option.series[0].data[i].value;
 	   }
    }
      console.log();
      getMousePos(clickName,clickNum,total);
        var a = this;
        setTimeout(function(){
...
//    });
    ...
    chart.on('click',function(e){
    	getMousePos(e.name,e.value,total);
     });
    
    $('body').click(function(e) {
    	   if($(e.target).attr("data-zr-dom-id") != "zr_100"){
    		   $("#showLenDataId").css("display","none");
    	   }
    	});
...
function getMousePos(clickName,clickNum,total) {
 e = arguments.callee.caller.arguments[0] || window.event;
e = event || window.event;
clickNum = parseInt(clickNum);
total = parseInt(total);
$("#showLenDataId").html(clickName +":"+clickNum + " ("+((clickNum/total*100).toFixed(2))+ "%)").css("display","block");
...
//			//年
			if(couYearTypeNum[str[6]][str[3]][str[8]] ==  undefined){
				couYearTypeNum[str[6]][str[3]][str[8]] = parseInt(str[2]);
			}else{
				couYearTypeNum[str[6]][str[3]][str[8]] += parseInt(str[2]);
			}
			//月
			if(couYearTypeNum[str[6]][str[3]][str[9]] ==  undefined){
				couYearTypeNum[str[6]][str[3]][str[9]] = parseInt(str[2]);
			}else{
				couYearTypeNum[str[6]][str[3]][str[9]] += parseInt(str[2]);
			}
